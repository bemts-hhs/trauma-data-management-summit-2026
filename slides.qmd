---
title: "Leveraging Risk-Adjusted Metrics for Targeted Trauma PI Efforts"
subtitle: "Risk-Adjusted Mortality Measurement"
...
# Risk Adjustment :white_check_mark:
...
<!-- Calculate W score over the years -->
```{r w_calc}

#| label: w_calc

w_overall <- trauma_patients |>
  tidyr::nest(-Year) |>
  dplyr::mutate(
...
# patient estimates
patients_saved <- paste0(
  "* Patients saved that were predicted to die in order of years: ",
  paste0(round(x = w_overall$Patient_Estimate), collapse = ", "),
  "."
)

# create a bar plot of the W score for each year
# plot the RMM data over time
w_overall_plot <- w_overall |>
  ggplot2::ggplot(ggplot2::aes(
    x = Year,
    y = W_Score,
    fill = "cornflowerblue",
    label = round(x = W_Score, digits = 2)
  )) +
  ggplot2::geom_col(
    alpha = 0.25,
    width = 0.6,
...
# State-level W Score 2020-2024

![](figures/w_overall_plot.png){fig-alt="A column chart by year from 2020 through 2024 of the calculated W-Score, with one column per year from 2020 through 2024. Count of patients that lived that were predicted to die are shown in the caption below the chart. Each column has the printed W-Score for each given year." fig-align="center"}

::: {.notes}

We examine first the W-Score, and then we will see how the RMM helps to provide a better picture given that it is not overly weighted by patients that are predicted to live.
...
# estimate # of patients lives saved over what was expected 2020-2024

# get predicted deaths
predicted_deaths <- w_overall |>
  dplyr::summarize(total = round(sum(Predicted_Deaths))) |>
  dplyr::pull(total)

# get observed deaths
observed_deaths <- w_overall |>
  dplyr::summarize(total = sum(N_Deaths)) |>
  dplyr::pull(total)

# get overall saved lives
overall_saved_lives <- w_overall |>
